...
## GOSK能做什么
- 任何能以 Markdown 文件形式存在的知识，都能获得完整的操作系统级支持：版本历史（git log）、探索分支（git branch）、人工审查（PR）、溯源追踪（git blame）、渐进式理解（git diff）。
- AI 智能体作为操作系统进程运行——它们提交议题、提议变更、响应人工审查。
- 人类判断充当调度器。未经你合并确认，任何内容都不得进入知识库。

---

## GOSK不是什么
- 不是数据库。Git 就是数据库。
- 不是 RAG 流水线。知识是编译出来的，不是检索出来的。
- 不绑定任何特定领域。你可以用它管理论文库、实验笔记本、公司维基，皆可。
